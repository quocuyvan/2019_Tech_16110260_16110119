...
const { AuthorBook, Author } = require('../models/db')
    ...
    if (queryString.length <= 2) {
        AuthorBook.count().then(numRow => {
            ...
            AuthorBook.findAll({
                ...
                return res.json(PagingResult(author_books, {
                    pageNumber: page,
                    pageSize: pageSize,
                    totalRows: totalRows,
        ...
        AuthorBook.count({ where: whereClause }).then(numRow => {
            const totalRows = numRow;
            const totalPages = Math.ceil(totalRows / pageSize);
            AuthorBook.findAll({
                where: whereClause,
...
router.get('/getByAuthor/:id(\\d+)', (req, res) => {
    let page = 0;
    if (req.query.p) page = parseInt(req.query.p);
    let pageSize = 20;
    if (req.query.s) pageSize = parseInt(req.query.s);
    ...
    if (queryString.length <= 2) {
        // conditions
        const whereClause = { authorId: req.params.id };
...
router.post('/', [
    ...
    AuthorBook.create(req.body).then(author_book => {
        return res.json(Result(author_book))
    }).catch(err => {
...
router.put('/:id(\\d+)', [
    ...
    AuthorBook.findByPk(req.params.id).then(author_book => {
        if (author_book != null) {
            author_book.update({
