    ...
    if (queryString.length <= 2) {
        Member.count().then(numRow => {
            ...
            Member.findAll({
                ...
                return res.json(PagingResult(members, {
                    pageNumber: page,
                    pageSize: pageSize,
                    totalRows: totalRows,
        ...
        Member.count({ where: whereClause }).then(numRow => {
            const totalRows = numRow;
            const totalPages = Math.ceil(totalRows / pageSize);
            Member.findAll({
                order: [
...
router.get('/:id', (req, res) => {
    Member.findByPk(req.params.id).then(type => {
        if (type != null) {
...
router.post('/', (req, res) => {
    Member.create(req.body).then(type => {
        res.json(Result(type))
    }).catch(err => {
