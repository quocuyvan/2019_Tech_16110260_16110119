...
const { Publisher } = require('./../models/db')
    ...
    if (queryString.length <= 2) {
        Publisher.count().then(numRow => {
            ...
            Publisher.findAll({
                ...
                return res.json(PagingResult(publishers, {
                    pageNumber: page,
                    pageSize: pageSize,
                    totalRows: totalRows,
        ...
        Publisher.count({ where: whereClause }).then(numRow => {
            const totalRows = numRow;
            const totalPages = Math.ceil(totalRows / pageSize);
            Publisher.findAll({
                order: [
...
router.get('/:id', (req, res) => {
    Publisher.findByPk(req.params.id).then(type => {
        if (type != null) {
...
router.post('/', (req, res) => {
    Publisher.create(req.body).then(type => {
        res.json(Result(type))
    }).catch(err => {
